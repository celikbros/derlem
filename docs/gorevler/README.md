...
| [TASK-002](TASK-002-contribution-task-type-registry.md) | Contribution task-type registry → **yeniden çerçevelendi:** omurga (`payload jsonb` + kanonik yayın) + ilk tip `response_edit_pair` | feature | DRAFT — release #1 ve tek karar (tipli/tipsiz alanlar) bekliyor. Panel notu: [katki_gorev_tipleri_karar_notu.md](../katki_gorev_tipleri_karar_notu.md). |
...
| [TASK-004](TASK-004-contribution-bundle-silent-loss.md) | Contribution bundling silently loses data and mislabels purpose | fix | **READY** — 2026-09-12. `contentPurposeForTaskType` korumasız varsayılan; `free_text` prompt'u ve katkı `domain`'i demette sessizce düşüyor. |
