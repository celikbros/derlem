...
| [TASK-002](TASK-002-contribution-task-type-registry.md) | Contribution backbone (`payload jsonb` + kanonik yayın + kanonik-okur worker) + ilk tip `response_edit_pair` — **Faz A** | feature | **IN PROGRESS** — Faz A, 7 dilim (S1–S7). **S1–S4 bitti**, 2026-09-13: worker kanonik kaydı okuyor; migration 000028; Go tip kayıt defteri + doğrulama; demet soru-cevabı ve düzeltme çiftini kanonik kayıt olarak, köken ve not dahil kayıpsız yazıyor (Go↔Python ortak fixture). Sıradaki S5: web formu. Panel notu: [katki_gorev_tipleri_karar_notu.md](../katki_gorev_tipleri_karar_notu.md). |
...
| 4 | Katkı omurgası + `response_edit_pair` (Faz A) | [TASK-002](TASK-002-contribution-task-type-registry.md) | **IN PROGRESS** — S4/7 bitti (2026-09-13) |
