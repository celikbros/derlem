...
| [TASK-002](TASK-002-contribution-task-type-registry.md) | Contribution backbone (`payload jsonb` + kanonik yayın + kanonik-okur worker) + ilk tip `response_edit_pair` — **Faz A** | feature | **IN PROGRESS** — Faz A, 7 dilim (S1–S7). **S1 bitti** 2026-09-13: worker kanonik kaydı okuyor (örnekleme, parmak izi, benzerlik); kontrol koşusu eski kodda 6 kırmızı. Sıradaki S2: migration 000028. Panel notu: [katki_gorev_tipleri_karar_notu.md](../katki_gorev_tipleri_karar_notu.md). |
...
| 4 | Katkı omurgası + `response_edit_pair` (Faz A) | [TASK-002](TASK-002-contribution-task-type-registry.md) | **IN PROGRESS** — S1/7 bitti (2026-09-13) |
