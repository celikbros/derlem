...
| [TASK-010](TASK-010-recovered-objects-and-raw-archive.md) | Recovered objects + custody of the Faz-2 raw sources | ops | **IN PROGRESS** — 2026-09-17. "Kalıcı kayıp" 422 nesne bulundu ve depoya kondu (422/422 yazdıktan sonra doğrulandı), kayıp listesi kapandı; 24,65 GB ham arşiv alındı ve SHA256SUMS ile doğrulandı (9/9 OK); 12,85 GB çift kopya ölçülüp silindi; "hiçbir metin yerinde temizlenmez" kuralı ve "ham arşiv yedek dışı" kararı yazıldı. Açık: 7 kaynağın kayda alınması, restore tatbikatı. |
| [TASK-012](TASK-012-clean-candidate-v3.md) | Clean candidate v3: quality v2, held-out split, near-dedup, rejection report | feature | **IN PROGRESS** — 2026-09-18. Kod + testler + 100k uçtan uca koşu bitti (249 sn; 99.007 aday / 43 held-out / 937 kalite / 13 yakın kopya). **Tam üretim bitti 2026-09-19** (~3,4 saat): eğitim adayı 5.827.650 satır / 11,90 GB (`83dcac77…c0059`), held-out 2.239 satır (`4a8595db…952c45`); atılan: PII 104.853, kalite 55.677, yakın kopya 35.833, dil (fastText, 3.13 ortamında liste) 1.492. SHA'lar ve held-out kuralı bağımsız doğrulandı; iki çıktı kaynak olarak kaydedildi, içe alma kuyrukta. Sıradaki: 200 örnek incelemesi (sahip). |
...
| [TASK-013](TASK-013-hacim-plani.md) | Volume plan: where the next tokens come from | plan | **READY (yalnız planlama)** — 2026-09-18. Kazıyıcı envanteri çıkarıldı; en büyük girdi kendi tarama değil Hugging Face veri seti indirmesi (C4/mC4 + Wikipedia). Hiçbir toplayıcı çalıştırılmadı; TASK-011'e bağlı. |
| [TASK-015](TASK-015-periodic-maintenance-sweep-inside-the-worker.md) | Periodic maintenance sweep inside the worker loop (TASK-014 follow-up) | fix | **IN REVIEW** — 2026-09-19 bitti: bakım taraması `run_forever` içinde 5 dk'da bir (sahte-saat testleri, kontrol koşusu kırmızı). Sahip worker'ı yeniden başlatınca DONE. |
| [TASK-016](TASK-016-parent-composition-by-raw-source-document.md) | Parent composition by raw source, document-boundary measurement, and the v2 rights contingency table | research | **DRAFT** — plan 2026-09-19, Faz 1, 1.5 gün |
| [TASK-017](TASK-017-hard-gate-no-pretrain-freeze-without.md) | Hard gate: no `pretrain` freeze without a registered eval/holdout reference source | fix | **IN REVIEW** — 2026-09-19 bitti: `QueueFreeze` pretrain'de eval/holdout referansı yoksa `eval_reference_missing`; entegrasyon testi + kontrol koşusu kırmızı; mevcut instruction testi etkilenmedi. Sahip API'yi yeniden başlatınca DONE. |
| [TASK-018](TASK-018-rehearse-pretrain-draft-freeze-txt-jsonl.md) | Rehearse pretrain draft → freeze → txt/jsonl export on the `_test` database with the 100k slice (Go half: draft + QueueFreeze; Python half: freeze + export jobs in-process) | delivery | **DRAFT** — plan 2026-09-19, Faz 1, 2 gün |
...
| [TASK-020](TASK-020-rejection-report-false-drop-audit-pack.md) | Rejection-report false-drop audit pack: stratified sheet (50 per reason) and byte-weighted scorer, no UI | ops | **DRAFT** — plan 2026-09-19, Faz 1, 1 gün |
...
| [TASK-022](TASK-022-document-boundary-policy-note-fragments-vs.md) | Document-boundary policy note (fragments vs whole documents) for new intake and any re-derivation | research | **DRAFT** — plan 2026-09-19, Faz 2, 0.5 gün |
| [TASK-023](TASK-023-licence-notes-for-turkish-hugging-face.md) | Licence notes for Turkish Hugging Face datasets in the TASK-011 format | research | **DRAFT** — plan 2026-09-19, Faz 2, 1 gün |
...
| [TASK-026](TASK-026-quality-filter-language-honesty-tr-web.md) | Quality-filter language honesty: `tr-web-*` policies write `not_evaluated` for non-Turkish sources | fix | **DRAFT** — plan 2026-09-19, Faz 3, 0.5 gün |
| [TASK-027](TASK-027-research-language-detection-options-that-run.md) | Research: language detection options that run in the Python 3.14 worker, measured against the fastText baseline | research | **DRAFT** — plan 2026-09-19, Faz 3, 0.5 gün |
...
| [TASK-011](TASK-011-ham-kaynak-hak-arastirmasi.md) | Rights research for the seven Faz-2 raw sources | research | **READY** — 2026-09-17 kurucu kararı: ham kaynaklar `unknown` ile kayda alınır (sürüme giremez), hak araştırması ürün çıkmadan önce yapılır. |
