...
| [TASK-002](TASK-002-contribution-task-type-registry.md) | Contribution backbone (`payload jsonb` + kanonik yayın + kanonik-okur worker) + ilk tip `response_edit_pair` — **Faz A** | feature | **IN PROGRESS** — Faz A, 7 dilim (S1–S7). **S1** (worker kanonik kaydı okuyor), **S2** (migration 000028: `payload`, `response_edit_pair`, köken) ve **S3** (Go tip kayıt defteri + gönderim doğrulaması; payload'lı ve model kökenli katkı S4'e kadar demetlenemez) bitti, 2026-09-13. Sıradaki S4: demetin kanonik JSONL yayını. Panel notu: [katki_gorev_tipleri_karar_notu.md](../katki_gorev_tipleri_karar_notu.md). |
...
| 4 | Katkı omurgası + `response_edit_pair` (Faz A) | [TASK-002](TASK-002-contribution-task-type-registry.md) | **IN PROGRESS** — S3/7 bitti (2026-09-13) |
