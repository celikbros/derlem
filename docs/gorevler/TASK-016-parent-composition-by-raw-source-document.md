...
- ~~Optional determinism check: fetch `scripts/rebuild_faz2_corpus.py` from `celikbros/Gardash`, fix paths, compare per-source counts with `per_source.in`; divergence reported, not hidden.~~ **Obsolete 2026-09-24:** `celikbros/Gardash` was deleted and the script has no other known copy. Not needed: the per-source document counts already match `per_source.in` exactly (7/7, measured 2026-09-17), and the hash attribution below covers 100 % of parent lines — the composition question is answered without the script. What is lost is only proof that the raw files regenerate the parent byte for byte; the parent object itself is intact.
