...
const sourceColumns = `
	...
	object_sha256, byte_size, line_count, document_count, detected_encoding,
	pii_status, risk_level, approval_status, version, created_by::text,
...
func scanSource(row scanner) (domain.Source, error) {
	...
	err := row.Scan(
		...
		&source.DetectedEncoding, &source.PIIStatus, &source.RiskLevel,
